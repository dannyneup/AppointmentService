    ...
    ITestOutputHelper outputHelper) : IntegrationTestBase(serviceTestFixture, keycloakTestFixture, outputHelper)
{
    ...
    public async Task Stream_WhenFilteredByInsuranceNumber_ShouldReturnFilteredPatients(StringFilter filter,
    ...
    [Theory]
    [MemberData(nameof(SharedFilterCases.StringCases), MemberType = typeof(SharedFilterCases))]
    public async Task Stream_WhenFilteredByName_ShouldReturnFilteredPatients(StringFilter filter,
        string name,
        bool expected)
    {
        var client = new PatientService.PatientServiceClient(Channel);
    ...
    private static async Task<IReadOnlyCollection<Patient>> ReadPatientStreamAsync(
        PatientService.PatientServiceClient client,
