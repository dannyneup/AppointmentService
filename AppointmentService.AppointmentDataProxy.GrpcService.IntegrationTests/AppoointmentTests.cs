    ...
    : IntegrationTestBase(serviceTestFixture, keycloakTestFixture, outputHelper)
{
    ...
    public async Task Update_WhenExternalDbReferenceEntitiesDoNotExist_ShouldThrowException()
    {
        ...
        var exception = await Assert.ThrowsAsync<RpcException>(async () =>
            await AppointmentServiceClient.UpdateAsync(
                new UpdateAppointmentRequest { Appointment = updatedAppointment }));
    ...
    public async Task Stream_WhenFilteredById_ShouldReturnFilteredAppointments(Int32Filter filter,
        int id,
        bool expected)
    {
        var client = new Protos.AppointmentService.AppointmentServiceClient(Channel);
    ...
    public async Task Stream_WhenFilteredByStart_ShouldReturnFilteredAppointments(GoogleTimestampFilter filter,
        Timestamp start,
        bool expected)
    {
        var appointment = new Appointment
        {
            Id = 123,
            Start = start,
            End = Timestamp.FromDateTimeOffset(start.ToDateTimeOffset()
                .AddHours(1)),
            PatientInsuranceNumber = "456",
            PracticeInstitutionCode = "789",
            TherapistId = _testTherapist.Id,
        ...
        await CreateTestIndividualRemedy();
        await AppointmentServiceClient.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments =
            await ReadAppointmentStreamAsync(AppointmentServiceClient, new AppointmentFilter { Start = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    [Theory]
    [MemberData(nameof(SharedFilterCases.TimestampCases), MemberType = typeof(SharedFilterCases))]
    public async Task Stream_WhenFilteredByEnd_ShouldReturnFilteredAppointments(GoogleTimestampFilter filter,
        Timestamp end,
        bool expected)
    {
        var appointment = new Appointment
        {
            Id = 123,
            Start = Timestamp.FromDateTimeOffset(end.ToDateTimeOffset()
                .AddHours(-1)),
            End = end,
            PatientInsuranceNumber = "456",
            PracticeInstitutionCode = "789",
        ...
        await AppointmentServiceClient.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments =
            await ReadAppointmentStreamAsync(AppointmentServiceClient, new AppointmentFilter { End = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    public async Task Stream_WhenFilteredByPatientInsuranceNumber_ShouldReturnFilteredAppointments(StringFilter filter,
        ...
        bool expected)
    {
        ...
        await AppointmentServiceClient.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments = await ReadAppointmentStreamAsync(AppointmentServiceClient,
            new AppointmentFilter { PatientInsuranceNumber = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    [Theory]
    [MemberData(nameof(SharedFilterCases.Int32Cases), MemberType = typeof(SharedFilterCases))]
    public async Task Stream_WhenFilteredByTherapistId_ShouldReturnFilteredAppointments(Int32Filter filter,
        int therapistId,
        bool expected)
    {
        var appointment = new Appointment
        ...
        await AppointmentServiceClient.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments =
            await ReadAppointmentStreamAsync(AppointmentServiceClient, new AppointmentFilter { TherapistId = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    [Theory]
    [MemberData(nameof(SharedFilterCases.StringCases), MemberType = typeof(SharedFilterCases))]
    public async Task Stream_WhenFilteredByPracticeInstitutionCode_ShouldReturnFilteredAppointments(StringFilter filter,
        string practiceInstitutionCode,
        bool expected)
    {
        var appointment = new Appointment
        {
        ...
        await AppointmentServiceClient.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments = await ReadAppointmentStreamAsync(AppointmentServiceClient,
            new AppointmentFilter { PracticeInstitutionCode = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    [Theory]
    [MemberData(nameof(SharedFilterCases.StringCases), MemberType = typeof(SharedFilterCases))]
    public async Task Stream_WhenFilteredByFixedRemedyDiagnosisCode_ShouldReturnFilteredAppointments(
        StringFilter filter,
        string fixedRemedyDiagnosisCode,
        bool expected)
    {
        var client = new Protos.AppointmentService.AppointmentServiceClient(Channel);
        ...
        await client.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments =
            await ReadAppointmentStreamAsync(client, new AppointmentFilter { FixedRemedyDiagnosisCode = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    [Theory]
    [MemberData(nameof(SharedFilterCases.Int32Cases), MemberType = typeof(SharedFilterCases))]
    public async Task Stream_WhenFilteredByIndividualRemedyId_ShouldReturnFilteredAppointments(Int32Filter filter,
        ...
        bool expected)
    {
        ...
        await CreateTestTherapist();
        await IndividualRemedyServiceClient.CreateAsync(new CreateIndividualRemedyRequest
            { IndividualRemedy = individualRemedy });
        await client.CreateAsync(new CreateAppointmentRequest { Appointment = appointment });
        var streamedAppointments =
            await ReadAppointmentStreamAsync(client, new AppointmentFilter { IndividualRemedyId = filter });
        var expectedAppointments = expected ? [appointment] : new List<Appointment>();
        Assert.Equivalent(streamedAppointments, expectedAppointments);
    }
    ...
    public async Task Stream_WhenAppointmentCountExceedsBatchSize_ShouldStreamAllAppointments()
    {
        ...
        var appointments = Enumerable
            ...
            .Select(iterator
                ...
                {
                    Id = iterator,
                    Start = Timestamp.FromDateTimeOffset(new DateTimeOffset(2025,
                        1,
                        1,
                        23,
                        49,
                        0,
                        new TimeSpan(-3, 0, 0))),
                    End = Timestamp.FromDateTimeOffset(
                        new DateTimeOffset(2025, 1, 1, 23, 49, 0, new TimeSpan(-2, 0, 0))),
                    PatientInsuranceNumber = "456",
                    PracticeInstitutionCode = "789",
                    TherapistId = _testTherapist.Id,
                    IndividualRemedyId = _testIndividualRemedy.Id
    ...
    private static async Task<IReadOnlyCollection<Appointment>> ReadAppointmentStreamAsync(
        Protos.AppointmentService.AppointmentServiceClient client,
        AppointmentFilter? filter = null)
    {
