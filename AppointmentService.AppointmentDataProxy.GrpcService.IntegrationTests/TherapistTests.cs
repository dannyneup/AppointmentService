    ...
    ITestOutputHelper outputHelper) : IntegrationTestBase(serviceTestFixture, keycloakTestFixture, outputHelper)
{
    ...
    public async Task Stream_WhenFilteredByName_ShouldReturnFilteredTherapists(StringFilter filter,
        string name,
        bool expected)
    {
        var client = new TherapistService.TherapistServiceClient(Channel);
    ...
    private static async Task<IReadOnlyCollection<Therapist>> ReadTherapistStreamAsync(
        TherapistService.TherapistServiceClient client,
        TherapistFilter? filter = null)
    {
